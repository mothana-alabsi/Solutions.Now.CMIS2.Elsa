...
namespace Solutions.Now.CMIS2.Elsa.Activities
{
    [Activity(
          Category = "Approval",
          DisplayName = "Final Evaluation Supervisor approvals",
          Description = "Add Users of Final Evaluation Supervisor approvals",
          Outcomes = new[] { OutcomeNames.Done }
      )]
    public class FinalEvaluationSupervisorUsers : Activity
    {
        private readonly Cmis2DbContext _cmis2DbContext;

        public FinalEvaluationSupervisorUsers(Cmis2DbContext cmis2DbContext)
        {
        ...
        {
            const int finalEvaluationSupervisor = 1894;
            ...
            List<WorkFlowRules> workFlowRules = _cmis2DbContext.WorkFlowRules.AsQueryable().Where(s => (s.workflow == finalEvaluationSupervisor && s.type == typeAction)).OrderBy(s => s.step).ToList<WorkFlowRules>();
            ...
            try
            {
                FinalEvaluationSupervisor finalEvaluation = await _cmis2DbContext.FinalEvaluationSupervisor.FirstOrDefaultAsync(i => i.serial == RequestSerial);
