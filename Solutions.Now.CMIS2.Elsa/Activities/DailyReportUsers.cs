...
namespace Solutions.Now.CMIS2.Elsa.Activities
{
    [Activity(
          Category = "Approval",
          DisplayName = "Daily Work Report approvals",
          Description = "Add Users of Daily Work Report approvals",
          Outcomes = new[] { OutcomeNames.Done }
      )]
    public class DailyReportUsers : Activity
    {
        private readonly Cmis2DbContext _cmis2DbContext;

        public DailyReportUsers(Cmis2DbContext cmis2DbContext)
        {
        ...
        protected override async ValueTask<IActivityExecutionResult> OnExecuteAsync(ActivityExecutionContext context)
        {
            ...
            List<WorkFlowRules> workFlowRules = _cmis2DbContext.WorkFlowRules.AsQueryable().Where(s => (s.workflow == 2966 && s.type == 441)).OrderBy(s => s.step).ToList<WorkFlowRules>();
            ...
            try
            {
                 DailyWorkReport dailyWorkReport = await _cmis2DbContext.DailyWorkReport.FirstOrDefaultAsync(i => i.serial == RequestSerial);
                Tender tender = await _cmis2DbContext.Tender.FirstOrDefaultAsync(s => s.tenderSerial.ToString().Equals(dailyWorkReport.tenderSerial.ToString()));
