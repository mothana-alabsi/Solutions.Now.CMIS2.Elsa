...
namespace Solutions.Now.CMIS2.Elsa.Activities
{
    [Activity(
          Category = "Approval",
          DisplayName = "Final Payment approvals",
          Description = "Add Users of Final Payment approvals",
          Outcomes = new[] { OutcomeNames.Done }
      )]
    public class FinalPaymentUsers : Activity
    {
        private readonly Cmis2DbContext _cmis2DbContext;

        public FinalPaymentUsers(Cmis2DbContext cmis2DbContext)
        {
        ...
        protected override async ValueTask<IActivityExecutionResult> OnExecuteAsync(ActivityExecutionContext context)
        {
            ...
            List<WorkFlowRules> workFlowRules = _cmis2DbContext.WorkFlowRules.AsQueryable().Where(s => (s.workflow == finalPayment && s.type == typeAction)).OrderBy(s => s.step).ToList<WorkFlowRules>();
