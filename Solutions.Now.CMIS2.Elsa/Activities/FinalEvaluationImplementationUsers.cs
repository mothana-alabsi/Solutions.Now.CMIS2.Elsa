...
namespace Solutions.Now.CMIS2.Elsa.Activities
{
    [Activity(
          Category = "Approval",
          DisplayName = "Final Evaluation Implementation approvals",
          Description = "Add Users of Final Evaluation Implementation approvals",
          Outcomes = new[] { OutcomeNames.Done }
      )]
    public class FinalEvaluationImplementationUsers : Activity
    {
        private readonly Cmis2DbContext _cmis2DbContext;

        public FinalEvaluationImplementationUsers(Cmis2DbContext cmis2DbContext)
        {
        ...
        {
            const int finalEvaluationImplementation = 664;
            ...
            List<WorkFlowRules> workFlowRules = _cmis2DbContext.WorkFlowRules.AsQueryable().Where(s => (s.workflow == finalEvaluationImplementation && s.type == typeAction)).OrderBy(s => s.step).ToList<WorkFlowRules>();
            ...
            try
            {
                FinalEvaluationImplementation finalEvaluation = await _cmis2DbContext.FinalEvaluationImplementation.FirstOrDefaultAsync(i => i.serial == RequestSerial);
                Tender tender = await _cmis2DbContext.Tender.FirstOrDefaultAsync(s => s.tenderSerial.ToString().Equals(finalEvaluation.tenderSerial.ToString()));
