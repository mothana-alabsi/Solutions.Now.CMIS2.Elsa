...
namespace Solutions.Now.CMIS2.Elsa.Activities
{
    [Activity(
          Category = "Approval",
          DisplayName = "Contractor Extention approvals",
          Description = "Add Users of Contractor Extention approvals",
          Outcomes = new[] { OutcomeNames.Done }
      )]
    public class ContractorExtentionUsers : Activity
    {
        private readonly Cmis2DbContext _cmis2DbContext;

        public ContractorExtentionUsers(Cmis2DbContext cmis2DbContext)
        {
        ...
        {
            const int constractorExtensionWF = 2911;
            ...
            List<WorkFlowRules> workFlowRules = _cmis2DbContext.WorkFlowRules.AsQueryable().Where(s => (s.workflow == constractorExtensionWF && s.type == typeAction)).OrderBy(s => s.step).ToList<WorkFlowRules>();
            ...
            try
            {
                ...
                Tender tender = await _cmis2DbContext.Tender.FirstOrDefaultAsync(s => s.tenderSerial.ToString().Equals(financialAndTimeExtension.tenderSerial.ToString()));
