...
namespace Solutions.Now.CMIS2.Elsa.Activities
{
    [Activity(
          Category = "Approval",
          DisplayName = "Consultant interim payment approvals",
          Description = "Add Users of Consultant interim payment approvals",
          Outcomes = new[] { OutcomeNames.Done }
      )]
    public class ConsultantInterimPaymentUsers : Activity
    {
        private readonly Cmis2DbContext _cmis2DbContext;

        public ConsultantInterimPaymentUsers(Cmis2DbContext cmis2DbContext)
        {
        ...
        {
            const int consultantInterimPayment = 552;
            ...
            List<WorkFlowRules> workFlowRules = _cmis2DbContext.WorkFlowRules.AsQueryable().Where(s => (s.workflow == consultantInterimPayment && s.type == typeAction)).OrderBy(s => s.step).ToList<WorkFlowRules>();
            ...
            try
            {
                ...
                Tender tender = await _cmis2DbContext.Tender.FirstOrDefaultAsync(s => s.tenderRef.ToString().Equals(invoices.Tender.ToString()));
