...
namespace Solutions.Now.CMIS2.Elsa.Activities
{
    [Activity(
          Category = "Approval",
          DisplayName = "Compulsory Labor Staff approvals",
          Description = "Add Users of Compulsory Labor Staff approvals",
          Outcomes = new[] { OutcomeNames.Done }
      )]
    public class CompulsoryLaborStaffUsers : Activity
    {
        private readonly Cmis2DbContext _cmis2DbContext;

        public CompulsoryLaborStaffUsers(Cmis2DbContext cmis2DbContext)
        {
        ...
        protected override async ValueTask<IActivityExecutionResult> OnExecuteAsync(ActivityExecutionContext context)
        {
            ...
            List<WorkFlowRules> workFlowRules = _cmis2DbContext.WorkFlowRules.AsQueryable().Where(s => (s.workflow == 498 && s.type == 441)).OrderBy(s => s.step).ToList<WorkFlowRules>();
