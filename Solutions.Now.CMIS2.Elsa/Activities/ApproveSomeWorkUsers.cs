...
namespace Solutions.Now.CMIS2.Elsa.Activities
{
    [Activity(
          Category = "Approval",
          DisplayName = "Approve Some Work models approvals",
          Description = "Add Users of Daily workflow models approvals",
          Outcomes = new[] { OutcomeNames.Done }
      )]
    public class ApproveSomeWorkUsers : Activity
    {
        private readonly Cmis2DbContext _cmis2DbContext;

        public ApproveSomeWorkUsers(Cmis2DbContext cmis2DbContext)
        {
        ...
        protected override async ValueTask<IActivityExecutionResult> OnExecuteAsync(ActivityExecutionContext context)
        {
            ...
            List<WorkFlowRules> workFlowRules = _cmis2DbContext.WorkFlowRules.AsQueryable().Where(s => (s.workflow == 2964 && s.type == 441)).OrderBy(s => s.step).ToList<WorkFlowRules>();
            ...
            try
            {
                ApproveSomeWork approveSomeWork = await _cmis2DbContext.ApproveSomeWork.FirstOrDefaultAsync(i => i.serial == RequestSerial);
                Tender tender = await _cmis2DbContext.Tender.FirstOrDefaultAsync(s => s.tenderSerial.ToString().Equals(approveSomeWork.tenderSerial.ToString()));
