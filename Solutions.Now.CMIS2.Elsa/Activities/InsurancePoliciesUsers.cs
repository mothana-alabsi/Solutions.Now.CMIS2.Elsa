...
namespace Solutions.Now.CMIS2.Elsa.Activities
{
    [Activity(
          Category = "Approval",
          DisplayName = "Insurance Policies approvals",
          Description = "Add Users of Insurance Policies approvals",
          Outcomes = new[] { OutcomeNames.Done }
      )]
    public class InsurancePoliciesUsers : Activity
    {
        private readonly Cmis2DbContext _cmis2DbContext;

        public InsurancePoliciesUsers(Cmis2DbContext cmis2DbContext)
        {
        ...
        protected override async ValueTask<IActivityExecutionResult> OnExecuteAsync(ActivityExecutionContext context)
        {
            ...
            List<WorkFlowRules> workFlowRules = _cmis2DbContext.WorkFlowRules.AsQueryable().Where(s => (s.workflow == insurance && s.type == typeAction)).OrderBy(s => s.step).ToList<WorkFlowRules>();
            ...
            try
            {
                InsurancePolicy insurancePolicy = await _cmis2DbContext.InsurancePolicy.FirstOrDefaultAsync(i => i.policySerial == RequestSerial);
                Tender tender = await _cmis2DbContext.Tender.FirstOrDefaultAsync(s => s.tenderSerial.ToString().Equals(insurancePolicy.tenderSerial.ToString()));
