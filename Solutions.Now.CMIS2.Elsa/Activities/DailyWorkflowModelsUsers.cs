...
namespace Solutions.Now.CMIS2.Elsa.Activities
{
    [Activity(
          Category = "Approval",
          DisplayName = "Daily workflow models approvals",
          Description = "Add Users of Daily workflow models approvals",
          Outcomes = new[] { OutcomeNames.Done }
      )]
    public class DailyWorkflowModelsUsers : Activity
    {
        private readonly Cmis2DbContext _cmis2DbContext;

        public DailyWorkflowModelsUsers(Cmis2DbContext cmis2DbContext)
        {
        ...
        protected override async ValueTask<IActivityExecutionResult> OnExecuteAsync(ActivityExecutionContext context)
        {
            ...
            List<WorkFlowRules> workFlowRules = _cmis2DbContext.WorkFlowRules.AsQueryable().Where(s => (s.workflow == 550 && s.type == 441)).OrderBy(s => s.step).ToList<WorkFlowRules>();
