...
namespace Solutions.Now.CMIS2.Elsa.Activities
{
    [Activity(
          Category = "Approval",
          DisplayName = "Raise surveyors approvals",
          Description = "Add Users of Raise Surveyors approvals",
          Outcomes = new[] { OutcomeNames.Done }
      )]
    public class RaiseSurveyorsUsers : Activity
    {
        private readonly Cmis2DbContext _cmis2DbContext;

        public RaiseSurveyorsUsers(Cmis2DbContext cmis2DbContext)
        {
        ...
        {
            const int raiseSurveyor = 495;
            const int typeAction = 441;
            const int directoratePosition = 468;
            const int sectionPosition = 469;
            const int adminstrationSurvey = 487;
            const int directorateSurvey = 488;
            const int sectionSurvey = 2876;
            ...
            List<WorkFlowRules> workFlowRules = _cmis2DbContext.WorkFlowRules.AsQueryable().Where(s => (s.workflow == raiseSurveyor && s.type == typeAction)).OrderBy(s => s.step).ToList<WorkFlowRules>();
            ...
            try
            {
                RaiseSurveyors raiseSurveyors = await _cmis2DbContext.RaiseSurveyors.FirstOrDefaultAsync(r => r.surveySerial == RequestSerial);
                Tender tender = await _cmis2DbContext.Tender.FirstOrDefaultAsync(s => s.tenderSerial.ToString().Equals(raiseSurveyors.tenderSerial.ToString()));
                TblUsers users = await _cmis2DbContext.TblUsers.FirstOrDefaultAsync(u => u.Section == tender.section && u.Position == sectionPosition);
                userNameDB[3] = userNameDB[15] = userNameDB[17] = users.Username;
                ...
                userNameDB[4] = userNameDB[14] = userNameDB[18] = users.Username;
                users = await _cmis2DbContext.TblUsers.FirstOrDefaultAsync(u => u.Directorate == directorateSurvey && u.Position == directoratePosition);
                userNameDB[7] = userNameDB[11] = userNameDB[21] = users.Username;
                users = await _cmis2DbContext.TblUsers.FirstOrDefaultAsync(u => u.Section == sectionSurvey && u.Position == sectionPosition);
                userNameDB[8] = userNameDB[10] = users.Username;
                users = await _cmis2DbContext.TblUsers.FirstOrDefaultAsync(u => u.Username.Equals(tender.communicationEng));
                userNameDB[2] = userNameDB[16] = users.Username;
                ...
                userNameDB[5] = userNameDB[13] = userNameDB[19] = users.Username;
                users = await _cmis2DbContext.TblUsers.FirstOrDefaultAsync(u => u.Administration == adminstrationSurvey && u.Position == adminstrationPosition);
                userNameDB[6] = userNameDB[12] = userNameDB[20] = users.Username;
            }
            catch (Exception ex)
            {
                ex.Message.ToString();
