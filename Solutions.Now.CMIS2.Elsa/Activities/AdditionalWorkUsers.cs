...
namespace Solutions.Now.CMIS2.Elsa.Activities
{
    [Activity(
          Category = "Approval",
          DisplayName = "Additional Work approvals",
          Description = "Add Users of Additional Work approvals",
          Outcomes = new[] { OutcomeNames.Done }
      )]
    public class AdditionalWorkUsers : Activity
    {
        private readonly Cmis2DbContext _cmis2DbContext;

        public AdditionalWorkUsers(Cmis2DbContext cmis2DbContext)
        {
        ...
        {
            const int additionalWorkWF = 2882;
            ...
            List<WorkFlowRules> workFlowRules = _cmis2DbContext.WorkFlowRules.AsQueryable().Where(s => (s.workflow == additionalWorkWF && s.type == typeAction)).OrderBy(s => s.step).ToList<WorkFlowRules>();
            ...
            try
            {
                AdditionalWork additionalWork = await _cmis2DbContext.AdditionalWork.FirstOrDefaultAsync(r => r.serial == RequestSerial);
                Tender tender = await _cmis2DbContext.Tender.FirstOrDefaultAsync(s => s.tenderSerial.ToString().Equals(additionalWork.tenderSerial.ToString()));
