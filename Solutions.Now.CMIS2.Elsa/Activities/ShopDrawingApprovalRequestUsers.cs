...
namespace Solutions.Now.CMIS2.Elsa.Activities
{
    [Activity(
          Category = "Approval",
          DisplayName = " Shop Drawing Approval models approvals",
          Description = "Add Users of Daily workflow models approvals",
          Outcomes = new[] { OutcomeNames.Done }
      )]
    public class ShopDrawingApprovalRequestUsers : Activity
    {
        private readonly Cmis2DbContext _cmis2DbContext;

        public ShopDrawingApprovalRequestUsers(Cmis2DbContext cmis2DbContext)
        {
        ...
        protected override async ValueTask<IActivityExecutionResult> OnExecuteAsync(ActivityExecutionContext context)
        {
            ...
            List<WorkFlowRules> workFlowRules = _cmis2DbContext.WorkFlowRules.AsQueryable().Where(s => (s.workflow == 2965 && s.type == 441)).OrderBy(s => s.step).ToList<WorkFlowRules>();
            ...
            try
            {
                ShopDrawingApprovalRequest shopDrawingApprovalRequest = await _cmis2DbContext.ShopdrawingApprovalRequest.FirstOrDefaultAsync(i => i.serial == RequestSerial);
                Tender tender = await _cmis2DbContext.Tender.FirstOrDefaultAsync(s => s.tenderSerial.ToString().Equals(shopDrawingApprovalRequest.tenderSerial.ToString()));
                //TblUsers users = await _cmis2DbContext.TblUsers.FirstOrDefaultAsync(u => u.ID == 3372);
                //userNameDB[0] = users.Username;
                //users = await _cmis2DbContext.TblUsers.FirstOrDefaultAsync(u => u.ID == 3373);
                //userNameDB[3] = users.Username;
                //users = await _cmis2DbContext.TblUsers.FirstOrDefaultAsync(u => u.ID == 3374);
                //userNameDB[2] = users.Username;
                //users = await _cmis2DbContext.TblUsers.FirstOrDefaultAsync(u => u.ID == 3375);
                //userNameDB[1] = users.Username;
                //users = await _cmis2DbContext.TblUsers.FirstOrDefaultAsync(u => u.ID == 3408);
