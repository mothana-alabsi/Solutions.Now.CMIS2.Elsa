...
namespace Solutions.Now.CMIS2.Elsa.Activities
{
    [Activity(
          Category = "Approval",
          DisplayName = "Contractor interim payment approvals",
          Description = "Add Users of Contractor interim payment approvals",
          Outcomes = new[] { OutcomeNames.Done }
      )]
    public class ContractorInterimPaymentUsers : Activity
    {
        private readonly Cmis2DbContext _cmis2DbContext;

        public ContractorInterimPaymentUsers(Cmis2DbContext cmis2DbContext)
        {
        ...
        {
            const int contractorInterimPayment = 551;
            ...
            List<WorkFlowRules> workFlowRules = _cmis2DbContext.WorkFlowRules.AsQueryable().Where(s => (s.workflow == contractorInterimPayment && s.type == typeAction)).OrderBy(s => s.step).ToList<WorkFlowRules>();
            ...
            try
            {
                Invoices invoices = await _cmis2DbContext.Invoices.FirstOrDefaultAsync(i => i.serial == RequestSerial);
                Tender tender = await _cmis2DbContext.Tender.FirstOrDefaultAsync(s => s.tenderSerial.ToString().Equals(invoices.Tender.ToString()));
