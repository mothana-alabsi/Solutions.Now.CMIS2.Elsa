...
namespace Solutions.Now.CMIS2.Elsa.Activities
{
    [Activity(
          Category = "Approval",
          DisplayName = "Site Memo1 approvals",
          Description = "Add Users of Site Memo1 approvals",
          Outcomes = new[] { OutcomeNames.Done }
      )]
    public class SiteMemo1Users : Activity
    {
        private readonly Cmis2DbContext _cmis2DbContext;

        public SiteMemo1Users(Cmis2DbContext cmis2DbContext)
        {
        ...
        protected override async ValueTask<IActivityExecutionResult> OnExecuteAsync(ActivityExecutionContext context)
        {
            ...
            try
            {
                SiteMemo siteMemo = await _cmis2DbContext.SiteMemo.FirstOrDefaultAsync(i => i.serial == RequestSerial);
                Tender tender = await _cmis2DbContext.Tender.FirstOrDefaultAsync(s => s.tenderSerial.ToString().Equals(siteMemo.tenderSerial.ToString()));
