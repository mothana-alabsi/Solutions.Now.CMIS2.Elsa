...
namespace Solutions.Now.CMIS2.Elsa.Activities
{
    [Activity(
          Category = "Approval",
          DisplayName = "Payment For Completion approvals",
          Description = "Add Users of Payment For Completion approvals",
          Outcomes = new[] { OutcomeNames.Done }
      )]
    public class PaymentForCompletionUsers : Activity
    {
        private readonly Cmis2DbContext _cmis2DbContext;

        public PaymentForCompletionUsers(Cmis2DbContext cmis2DbContext)
        {
        ...
        {
            const int paymentForCompletion = 1856;
            ...
            List<WorkFlowRules> workFlowRules = _cmis2DbContext.WorkFlowRules.AsQueryable().Where(s => (s.workflow == paymentForCompletion && s.type == typeAction)).OrderBy(s => s.step).ToList<WorkFlowRules>();
