...
namespace Solutions.Now.CMIS2.Elsa.Activities
{
    [Activity(
          Category = "Approval",
          DisplayName = "Work Schedule approvals",
          Description = "Add Users of Work Schedule approvals",
          Outcomes = new[] { OutcomeNames.Done }
      )]
    public class WorkScheduleUsers : Activity
    {
        private readonly Cmis2DbContext _cmis2DbContext;

        public WorkScheduleUsers(Cmis2DbContext cmis2DbContext)
        {
        ...
        protected override async ValueTask<IActivityExecutionResult> OnExecuteAsync(ActivityExecutionContext context)
        {
            ...
            List<WorkFlowRules> workFlowRules = _cmis2DbContext.WorkFlowRules.AsQueryable().Where(s => (s.workflow == 548 && s.type == 441)).OrderBy(s => s.step).ToList<WorkFlowRules>();
