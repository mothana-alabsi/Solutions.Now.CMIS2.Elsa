...
namespace Solutions.Now.CMIS2.Elsa.Activities
{
    [Activity(
          Category = "Approval",
          DisplayName = "Sample Approve Office models approvals",
          Description = "Add Users of Daily workflow models approvals",
          Outcomes = new[] { OutcomeNames.Done }
      )]
    public class SampleApprovalOfficeFormUsers : Activity
    {
        private readonly Cmis2DbContext _cmis2DbContext;

        public SampleApprovalOfficeFormUsers(Cmis2DbContext cmis2DbContext)
        {
        ...
        protected override async ValueTask<IActivityExecutionResult> OnExecuteAsync(ActivityExecutionContext context)
        {
            ...
            List<WorkFlowRules> workFlowRules = _cmis2DbContext.WorkFlowRules.AsQueryable().Where(s => (s.workflow == 2959 && s.type == 441)).OrderBy(s => s.step).ToList<WorkFlowRules>();
            ...
            try
            {
                ...
                //TblUsers users = await _cmis2DbContext.TblUsers.FirstOrDefaultAsync(u =>u.ID == 3373);
                ...
                TblUsers users = await _cmis2DbContext.TblUsers.FirstOrDefaultAsync(u => u.Contractor == tender.tenderContracter1 && u.Position == 170);
                userNameDB[0] = users.Username;
                users = await _cmis2DbContext.TblUsers.FirstOrDefaultAsync(u => u.ID == 3408);
                userNameDB[2] = users.Username;
