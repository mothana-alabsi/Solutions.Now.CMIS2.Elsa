...
namespace Solutions.Now.CMIS2.Elsa.Activities
{
    [Activity(
          Category = "Approval",
          DisplayName = "The first installment approvals",
          Description = "Add Users of The first Installment approvals",
          Outcomes = new[] { OutcomeNames.Done }
      )]
    public class TheFirstInstallmentUsers : Activity
    {
        private readonly Cmis2DbContext _cmis2DbContext;

        public TheFirstInstallmentUsers(Cmis2DbContext cmis2DbContext)
        {
        ...
        {
            const int theFirstInstallment = 512;
            ...
            List<WorkFlowRules> workFlowRules = _cmis2DbContext.WorkFlowRules.AsQueryable().Where(s => (s.workflow == theFirstInstallment && s.type == typeAction)).OrderBy(s => s.step).ToList<WorkFlowRules>();
